...
| Private duplicate identity | Authenticated paged locator buckets, canonical per-replica quotas, amplification detection, and semantic reverse indexes preserve admitted offline duplicates without unbounded allocation or locator-based identity collapse | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
...
| Compartment movement | Cross-compartment copy or move creates a new target commitment, locator, encryption instance, DEK, selector, and policy decision before source removal | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
...
| Erasure state machine | Destruction intent and idempotency tokens commit before dispatch; ambiguous provider outcomes remain `DestructionUncertain`, and `KeysDestroyed` requires durable evidence for every recovery path | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
