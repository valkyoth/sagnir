...
| Private duplicate identity | A uniquely represented composite-key authenticated index gives logarithmic proofs; stable logical leaves exclude mutable placement, deterministic private logical roots are separately keyed and signed/checkpointed, and randomized encrypted envelopes remain public-storage projections | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
| Offline aggregate quotas | Signed escrow rights support offline admission; retirement requires surrender, an acknowledged spent root, explicit cutoff, or rights burning, while overdraw ratification creates new history and private quota metadata remains encrypted | Planned | `docs/protocol.md`, `docs/VERSION_PLAN.md` |
...
| Compartment movement | Merkle-chunked recursive translation uses typed leaf-equality/container-isomorphism proofs, least-privilege bridge disclosure, separately typed neutral commitments, sparse/redacted refusal, source/target/policy CAS, and atomic final commit | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
