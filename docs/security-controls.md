...
| Private duplicate identity | Persistent authenticated locator search trees provide logarithmic proofs and bounded amplification; per-replica and actor/device quotas carry across incarnations and locator rotations | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
...
| Compartment movement | Recursive graph translation rebuilds every compartment-bound descendant under source/target/policy CAS and proves no source identity remains reachable from the target | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
