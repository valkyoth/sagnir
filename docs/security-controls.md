...
| Private duplicate identity | Bounded locator buckets and authenticated semantic reverse indexes preserve independently signed offline duplicates without locator-based identity collapse | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
...
| Erasure state machine | Key destruction has an explicit irreversible point; recovery is forward-only and local erasure, controlled copies, remote acknowledgements, and uncontrolled residuals are reported separately | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
