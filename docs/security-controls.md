...
| Private duplicate identity | A uniquely represented composite-key authenticated index keys every semantic commitment and encryption instance separately; duplicate-identity and instance-fanout rights are distinct, stable logical leaves exclude mutable placement, and roots are separately keyed and signed/checkpointed | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
| Private root composition | Count-hiding opaque realm manifests compose compartment roots and support scoped inclusion/continuity proofs without disclosing unrelated compartments; placement projections remain endpoint-local | Planned | `docs/protocol.md`, `docs/VERSION_PLAN.md` |
| Offline aggregate quotas | Signed escrow rights support offline admission; expiry uses authoritative causal/checkpoint time, retirement requires surrender, an acknowledged spent root, explicit cutoff, or rights burning, while overdraw ratification creates new history and private quota metadata remains encrypted | Planned | `docs/protocol.md`, `docs/VERSION_PLAN.md` |
