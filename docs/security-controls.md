...
| Local store init | `.saga/` layout creation, dry-run planning, canonical root refusal, handle-relative Unix I/O, no-follow path traversal, owner-only Unix permissions, bounded complete reads, idempotent init, cross-platform OS file locking, and interrupted-init temp cleanup | Active | `sagnir-store`, `sagnir-cli` |
...
| Metadata persistence | Owner-only temporary files, file sync, handle-relative atomic rename, Unix directory sync, regular-file checks, and hard-link refusal protect realm/config writes; corruption fails closed with backup-first recovery guidance | Active | `sagnir-cli`, `docs/local-store.md` |
