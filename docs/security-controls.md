...
| Private duplicate identity | A canonical composite-key authenticated B+ tree gives logarithmic proofs; deterministic private logical roots are separated from randomized encrypted envelopes and public storage IDs | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
| Offline aggregate quotas | Signed escrow rights assigned to replica incarnations support offline admission; transfer is causal and merge-time overdraw or double-spend remains explicit quarantine | Planned | `docs/protocol.md`, `docs/VERSION_PLAN.md` |
...
| Compartment movement | Merkle-chunked recursive translation uses typed leaf-equality/container-isomorphism proofs, sparse/redacted refusal, source/target/policy CAS, and atomic final commit | Planned | `docs/vault-encryption.md`, `docs/VERSION_PLAN.md` |
