...
- owned secret-descriptor framing is exactly little-endian `u32 byte_length`
  ...
  input;
- generated human-recoverable bootstrap secrets use exactly 32 OS-CSPRNG bytes
  encoded as ASCII `saga1_` followed by RFC 4648 base64url without padding; the
  complete canonical ASCII representation is the KDF input across terminal,
...
- transition authorization/signature is over the complete old/new pair and
  governance context; signer/provider failure, cancellation, lost response, or
  invalid signature leaves prepared pages/nodes unreachable and the old head
  authoritative;
...
- tests cover concurrent updates, signer failure, duplicate event, event without
  ...
  node, and interruption between every publication boundary.
...
- the external profile accepts only the v0.98.2 `ProviderKeyHandleSource`
  ...
  terminal value, descriptor value, or generated recoverable secret cannot be
...
- tests cover profile substitution/downgrade, plaintext fallback, stale/replayed
  ...
  header, bootstrap/realm-key aliasing, passphrase purpose-separation and reuse-
  warning output, ordinary operation before cutover, old-key early deletion,
  stable assurance labels, offline-guess verifier fixture, and unsupported-
  platform refusal.
...
- secret-input composition covers interactive terminal state, owned descriptors,
  ...
  encoding, and non-convertible local-KDF/provider-handle source classes;
- transparency composition keeps history-independent current-state map proofs
  ...
  or leaves the complete prior state authoritative;
...
- v0.98.2 PTY/console, owned-descriptor, credential-provider, generated-secret,
  ...
  generated-secret encoding, cross-source byte equivalence, and source/profile
  incompatibility before secret acquisition;
- transparency current-map and append-event-log independent vectors, atomic
  ...
  jointly governed algorithm/version-transition tests;
...
- v0.98.2 secret-source API/compile-fail tests, forbidden argv/environment/
  ...
  canonical generated-secret encoding, cross-source equivalence, source/profile
  type refusal before acquisition, CSPRNG failure, cancellation/unwind, and
...
- v0.99.1 key transitions update the current map, append log, and dual-root state
  ...
  old state or complete new state and never a split authoritative pair;
...
- no-echo terminal, ownership-checked descriptor, admitted credential-provider,
  ...
  KDF/provider-handle compatibility are normative;
...
- separate authenticated key-transparency current-state map and append-only event
  ...
  state head atomically through one expected-head WAL transition;
