...
test -f security/pentest/v0.10.0.md
...
if grep 'path = "../crates/sagnir' fuzz/Cargo.toml |
    grep -v "version = \"$workspace_version\"" >/dev/null 2>&1; then
    echo "fuzz path dependencies must include explicit version = \"$workspace_version\"" >&2
    exit 1
fi
